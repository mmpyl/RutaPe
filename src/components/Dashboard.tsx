...
} from '../shared/selectors/carrierMetrics';
...
const Dashboard: React.FC<DashboardProps> = ({ orders, drivers, onNewOrder, onViewAllOrders, onViewRoutes }) => {
  ...
  const weeklyPerformance = useMemo(() => getWeeklyPerformanceData(orders), [orders]);
  const dashboardDate = useMemo(() => getDashboardDateLabel(), []);
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 capitalize">{dashboardDate}</p>
        </div>
        <button
          onClick={onNewOrder}
        ...
        <div className="lg:col-span-2 bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm">
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Serie calculada desde POD y tiempos relativos del snapshot actual</p>
            </div>
            <button
              onClick={onViewAllOrders}
              className="text-xs font-bold text-emerald-600 bg-emerald-50 rounded-lg px-3 py-2 hover:bg-emerald-100 transition-colors flex items-center gap-2"
            >
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={weeklyPerformance}>
                ...
                />
                <Legend wrapperStyle={{ fontSize: 12 }} />
                <Area type="monotone" dataKey="entregas" name="Entregas" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorEntregas)" />
                <Area type="monotone" dataKey="incidencias" name="Incidencias" stroke="#f97316" strokeWidth={2} fillOpacity={1} fill="url(#colorIncidencias)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
