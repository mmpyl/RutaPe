...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const mostIncidentsCarrier = [...carrierPerformance].sort(
    (a, b) => b.delayedOrders - a.delayedOrders,
  )[0];
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 max-w-3xl mt-1">
              Compara el cumplimiento de promesa, el tiempo promedio de entrega y el costo medio por
              despacho entre carriers para decidir a quién asignar más volumen.
            </p>
          </div>
          <div className="flex flex-wrap gap-3 text-xs font-bold text-slate-500">
            <div className="px-3 py-2 rounded-xl bg-slate-50 border border-slate-100">
              Promedio de entrega basado en pedidos completados y activos
            </div>
            <div className="px-3 py-2 rounded-xl bg-slate-50 border border-slate-100">
              SLA = % de pedidos sin retraso registrado
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="rounded-3xl bg-emerald-50 border border-emerald-100 p-5">
            ...
            <div className="text-2xl font-black text-slate-900">
              {leadingCarrier?.carrier ?? 'Sin datos'}
            </div>
            <div className="text-sm text-slate-500 mt-1">
              {leadingCarrier
                ? `${leadingCarrier.sla}% SLA · ${formatDuration(leadingCarrier.avgDeliveryMinutes)} promedio`
                : 'Aún no hay carriers suficientes para comparar'}
            </div>
          </div>
          <div className="rounded-3xl bg-blue-50 border border-blue-100 p-5">
            ...
            <div className="text-2xl font-black text-slate-900">
              {lowestCostCarrier?.carrier ?? 'Sin datos'}
            </div>
            <div className="text-sm text-slate-500 mt-1">
              {lowestCostCarrier
                ? `${formatCurrency(lowestCostCarrier.avgCost)} promedio por envío`
                : 'Sin volumen suficiente para análisis de costo'}
            </div>
          </div>
          <div className="rounded-3xl bg-amber-50 border border-amber-100 p-5">
            ...
            <div className="text-2xl font-black text-slate-900">
              {mostIncidentsCarrier?.carrier ?? 'Sin datos'}
            </div>
            <div className="text-sm text-slate-500 mt-1">
              {mostIncidentsCarrier
                ? `${mostIncidentsCarrier.delayedOrders} incidencias · ${mostIncidentsCarrier.sla}% SLA`
              ...
              <div key={carrier.carrier} className="rounded-3xl border border-slate-100 bg-slate-50 p-5">
                <div className="flex items-center justify-between gap-4 mb-4">
                  <div className="flex items-center gap-3">
                    <div
                      className="w-11 h-11 rounded-2xl text-white font-black text-xs flex items-center justify-center shadow-sm"
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-slate-400">
                        {carrier.totalOrders} pedidos evaluados
                      </div>
                    </div>
                  </div>
                  <span
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div className="bg-white rounded-2xl p-3 border border-slate-100">
                    <div className="text-lg font-black text-slate-900">
                      {formatDuration(carrier.avgDeliveryMinutes)}
                    </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50">
                {['ID', 'Cliente', 'Carrier', 'Estado', 'Valor', 'Acción'].map((h) => (
                  <th
                    key={h}
